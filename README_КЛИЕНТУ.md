...
> **Несколько устройств (телефон и эмулятор):** список сотрудников, которых создаёт администратор в приложении, хранится **на этом устройстве** (локальная база). На другом телефоне или в эмуляторе — **другая** база: тех же пользователей там не будет, пока их не создадут заново на том устройстве. Чтобы «видеть друг друга» в чате, на **обоих** устройствах должны быть учётные записи с **разными** логинами, заведённые **на каждом устройстве** (или позже — через общий сервер/облако, если так настроит разработчик). Тексты сообщений в личном чате могут синхронизироваться через **Supabase**, если в проекте заданы URL и ключ в `local.properties` и настроены таблицы (см. техническую документацию для разработчика).

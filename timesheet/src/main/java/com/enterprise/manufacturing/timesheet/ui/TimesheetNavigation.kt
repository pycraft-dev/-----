...
fun TimesheetTimerRoute(navController: NavHostController) {
    ...
    var hoursStr by rememberSaveable { mutableStateOf("") }
    var minutesStr by rememberSaveable { mutableStateOf("") }
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { ev ->
            when (ev) {
                ...
                TimesheetTimerEvent.InvalidDuration ->
                    snackbarHostState.showSnackbar(context.getString(R.string.timesheet_err_invalid_duration))
    ...
    TimesheetTimerScreen(
        ...
        hoursStr = hoursStr,
        onHoursChange = { hoursStr = it.filter { c -> c.isDigit() }.take(4) },
        minutesStr = minutesStr,
        onMinutesChange = { minutesStr = it.filter { c -> c.isDigit() }.take(2) },
        onAddTask = {
            viewModel.addTask(
...
private fun TimesheetTimerScreen(
    snackbarHostState: SnackbarHostState,
    recentEntries: List<TimesheetHistoryRow>,
    taskTitle: String,
        ...
        LazyColumn(
            ...
            item {
                OutlinedTextField(
                    ...
                    label = { Text(stringResource(R.string.timesheet_task_what_done)) },
                    singleLine = false,
                    minLines = 2,
                )
            }
            item {
                Row(
                    ...
                    OutlinedTextField(
                        ...
                        label = { Text(stringResource(R.string.timesheet_hours_hint)) },
                    ...
                    OutlinedTextField(
                        ...
                        label = { Text(stringResource(R.string.timesheet_minutes_hint)) },
                        keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
                        singleLine = true,
                    )
                }
            ...
            item {
                Button(
                    ...
                    Text(stringResource(R.string.timesheet_add_task))
                }
            }
            ...
            if (recentEntries.isNotEmpty()) {
                item {
                    ...
                    Text(
                        text = stringResource(R.string.timesheet_recent_title),
                        style = MaterialTheme.typography.titleMedium,
                    )
                }
                items(recentEntries, key = { it.id }) { row ->
